function IDataSet = BLK(blkfile,zfn,tfn,type,method,framemethod)
% BLK.m
% 2010-03-09 Zhang Li
% Convert Optic Imaging,Inc. Block File, analysis zero frame and stimulus response
...
for i = 1:IDataSet.head.ntrials
    for s = 1:IDataSet.head.nstimuli
        ...
        if strcmp(framemethod,'sum')
            IDataSet.image{i,s} = squeeze(sum(rf));
        else
            IDataSet.image{i,s} = squeeze(mean(rf));
        end
    end
end
close(hwaitbar);
