% ProcessMarker_Event.m %
% Process Markers according to the "Marker Encode Protocal"

disp('  Processing Marker ...');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
marker=round(marker*1000); % Convert Marker Time from "sec" to "ms" 
m=length(marker);     % Get the number of marker Triggers
...
stimulus_last_max=round(max(stimulus_last));  % Integer max time of stimulus last(ms)

clear encode_time;
clear stimulus_last;

encode_max=max(stimulus_code);
nStimuli=floor((encode_max)/5)+1; % 0-5 ms for Blank Control ,ClockWise Stimuli
